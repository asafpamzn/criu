...
# Basic write-protect (UFFD_FEATURE_PAGEFAULT_FLAG_WP) is in 5.7+ but insufficient for CLONE mode.
```
...
# Switch to CLONE branch (if not on main)
git checkout criu-clone
...
```bash
# Check that CRIU source has CLONE support
grep "clone_dump" ./criu/config.c
# Should show multiple lines with clone_dump

# Note: --clone-dump is not listed in --help but works
...
### CLONE Run Order (Current Implementation)
...
5. CRIU prints `PAGE SERVER READY TO SERVE`, resumes the source, and runs
   `cr_lazy_mem_dump()` (page server + unified sender thread) while CLONE tracking
   stays active.
...
CLONE dump requires shared storage accessible from both PRIMARY and REPLICA for
...
- Script flow:
  ...
  - Replica runs `criu lazy-pages` + `criu restore` **with `--clone-dump`**.

**After the run:**
- Replica Valkey should respond (`valkey-cli ping`).
- If it hangs:
  - Check `/fsx/lazy/lazy-restore.log` and `/fsx/lazy/lazy-server.log`.
  - **Symptom of missing `--clone-dump`:** `lazy-restore.log` empty and `lazy-server.log`
    ends with `page_server_start_read`. Fix by ensuring `--clone-dump` is present in
...
```bash
# Verbose dump
sudo ./criu/criu dump -t $PID -D /tmp/images -v4 --clone-dump --lazy-pages
...
| `criu/clone-dump.c` | CLONE dump implementation |
| `criu/include/clone-dump.h` | CLONE dump API |
| `criu/page-xfer.c` | Page transfer with CLONE integration |

---

## 14. Quick Reference Commands

**Note:** Some CLONE-related flags (`--clone-dump`, `--ext-unix-sk`) are not shown in
`--help` but are functional. They were added for the CLONE dump feature.
