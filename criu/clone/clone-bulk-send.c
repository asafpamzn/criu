...
wait_for_freeze:
	...
	/* Final scan after freeze - each scanner handles its portion */
	{
		 ...
		 */
		if (scanner_id == 0 && clone_bpf_active()) {
			struct clone_bpf_region *bpf_regions;
			unsigned long bpf_pages = 0;
			int bpf_nr, i;

			use_bpf = true;
			bpf_regions = xmalloc(CLONE_PAGEMAP_SCAN_VEC_LEN * sizeof(*bpf_regions));
			if (bpf_regions) {
...
int clone_bpf_drain_to_queues(void)
{
	...
	if (!g_using_bpf_mode || !clone_bpf_active()) {
		pr_err("clone_bpf_drain_to_queues called but BPF not active\n");
		return -1;
	}
	 ...
	 */
	if (clone_bpf_drain_addrs(&bpf_addrs, &bpf_addr_count) < 0) {
	...
	regs = xmalloc(CLONE_PAGEMAP_SCAN_VEC_LEN * sizeof(*regs));
	if (!regs) {
		close(pagemap_fd);
		goto skip_scan_merge;
	}
	...
	list_for_each_entry(lve, lazy_vmas, list) {
		...
		args.vec = (u64)(unsigned long)regs;
		args.vec_len = CLONE_PAGEMAP_SCAN_VEC_LEN;
		args.max_pages = CLONE_PAGEMAP_SCAN_MAX_PAGES_ITER1;
		args.category_anyof_mask = PAGE_IS_WRITTEN;
		args.return_mask = PAGE_IS_WRITTEN;
...
skip_scan_merge:
	...
	max_regions = (event_count > 0) ? (int)event_count : CLONE_PAGEMAP_SCAN_VEC_LEN;
...
void clone_signal_last_scan(void)
{
	 ...
	 */
	if (g_using_bpf_mode) {
		int dirty_pages = clone_bpf_drain_to_queues();
		if (dirty_pages < 0) {
			pr_err("BPF drain failed!\n");
			BUG();
		}
