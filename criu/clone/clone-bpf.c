/*
 * eBPF-based dirty page tracker for CLONE migration.
 *
 ...
 *
 * Lifecycle:
 *   1. clone_bpf_start(pid)  — load BPF, attach kfunc, start collecting
 *   2. clone_bpf_drain(...)  — drain ring → sorted, dedup'd region list
 *   3. clone_bpf_stop()      — detach + free
 */
#include <stdlib.h>
#include <string.h>
#include <errno.h>
 ...
 */
unsigned long *clone_bpf_get_initial_dirty(unsigned long *count)
{
...
int clone_bpf_start(pid_t target_pid)
{
 ...
 */
int clone_bpf_drain(struct clone_bpf_region *out_regions, int max_regions,
		  unsigned long *out_count)
{
	...
	*out_count = 0;
	...
	dc.cap = CLONE_BPF_DRAIN_INITIAL_CAP;
	dc.count = 0;
 ...
 */
int clone_bpf_drain_addrs(unsigned long **out_addrs, unsigned long *out_count)
{
	struct ring_buffer *rb;
	struct drain_ctx dc;
	int err, i, unique;
