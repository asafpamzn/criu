...
/*
 * COW memory helpers — extracted to keep COW logic separate from
 * CRIU's standard page dump path.
 */

/* Register a lazy VMA for COW bulk transfer (skips per-page pagemap scan) */
static int cow_register_lazy_vma(struct pstree_item *item,
				 struct vma_area *vma)
{
	...
	pr_debug("cow lazy VMA 0x%llx-0x%llx (%lu pages, dst=%lu)\n",
...
/* Count pages in non-lazy VMAs (for page-pipe sizing in COW mode) */
static unsigned int cow_count_non_lazy_pages(struct pstree_item *item,
					     struct vm_area_list *vma_list)
{
	struct vma_area *vma_area;
	unsigned long pages = 0;

	list_for_each_entry(vma_area, &vma_list->h, list) {
		bool cow_tracked, lazy_capable;
	...
	return pages ? (pages < UINT_MAX ? pages : UINT_MAX) : 1;
}
			 ...
			 bool has_parent, struct page_xfer *xfer)
{
	...
	if (opts.cow_dump && lazy_capable)
		return cow_register_lazy_vma(item, vma);
					...
					struct parasite_ctl *ctl)
{
	...
	if (opts.cow_dump && mdc->lazy)
		nr_segs = cow_count_non_lazy_pages(item, vma_area_list);
