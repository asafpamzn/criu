#ifndef __CR_CLONE_BPF_H__
#define __CR_CLONE_BPF_H__

#include <stdbool.h>
#include "types.h"

/*
 * Region returned by clone_bpf_drain — same layout as converge_region
 * in page-xfer.c so it can be used directly with dispatch functions.
 */
struct clone_bpf_region {
	u64 start;
	u64 end;
	u64 categories;
};
 ...
 */
extern int clone_bpf_start(pid_t target_pid);
 ...
 */
extern int clone_bpf_drain(struct clone_bpf_region *out_regions,
 ...
 */
extern u64 clone_bpf_drop_count(void);

/*
 * Stop BPF tracking — detach + free.
 */
extern void clone_bpf_stop(void);

/*
 * Check if BPF tracking is active.
 */
extern bool clone_bpf_active(void);
 ...
 */
extern int clone_bpf_drain_addrs(unsigned long **out_addrs, unsigned long *out_count);
 ...
 */
extern unsigned long *clone_bpf_get_initial_dirty(unsigned long *count);
#endif

#endif /* __CR_CLONE_BPF_H__ */
