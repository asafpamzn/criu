# CRIU COW Live Migration for Valkey
...
```
  ...
  ┌─────▼──────────────────────┐
  │  1. SEIZE (26ms frozen)    │    ┌──────────────────────────┐
  │  - ptrace-stop all threads │───▶│  Start restore.sh        │
  ...
  └─────┬──────────────────────┘
        │
  ┌─────▼──────────────────────┐
  │  2. TRACK (35ms, running)  │
  │  - WP_ASYNC on all VMAs    │
  │  - eBPF attach (no gap)    │
  │  - Sigacts capture (2ms)   │
  └─────┬──────────────────────┘
        │
  ┌─────▼──────────────────────┐    ┌──────────────────────────┐
  │  3. TRANSFER (~30s running)│    │  RECEIVE                 │
  ...
  └─────┬──────────────────────┘    └──────────┬───────────────┘
        │                                      │
  ┌─────▼──────────────────────┐               │
  │  4. CONVERGE (29ms frozen) │───────────────┘
  │  - SIGSTOP (verified poll) │    (sends dirty + non-lazy pages)
  │  - eBPF ring drain (~0ms)  │
  │  - Regs + FDs (6ms)       │───▶  converge_regs.dat, etc.
  │  - Dirty pages dispatch    │───▶  Overwrites stale pages
  │  - Non-lazy re-send        │───▶  127 pages (stacks + .data)
  │  - VMA diff (frozen scan)  │───▶  Injects mmap
  │  - SIGCONT source          │
  └─────┬──────────────────────┘
        │
  ┌─────▼──────────────────────┐    ┌──────────────────────────┐
  │  5. HANDOFF (0ms)          │    │  HANDOFF                 │
  │  - Send TCP "GO" ──────────│───▶│  - Apply converge regs   │
  │                            │    │  - Apply converge sigacts│
  │                            │    │  - PTRACE_DETACH all     │
  │                            │    │  - Valkey is live!       │
  └────────────────────────────┘    └──────────────────────────┘
```
...
At converge, the page server drains the ring in microseconds — O(dirty_pages)
instead of walking all 50M+ page table entries with PAGEMAP_SCAN
(O(total_pages), ~150ms). This reduced converge from 170ms to 29ms.
...
are not WP-tracked. They're re-sent from the frozen source at converge for
...
| CPU registers | converge capture + apply via PTRACE_SETREGSET |
| File descriptors | CRIU image files |
| TCP sockets | Closed (`--tcp-close`), clients reconnect |
| Signal handlers | converge capture (direct ptrace) + apply after detach |
...
**Source unavailability: 55ms total** (26ms seize + 29ms converge, two separate windows).

### Stage Timing (100GB + live traffic)

| Stage | Duration | Notes |
|-------|----------|-------|
| Seize | **26 ms** | Seize + maps + parasite + pagemap |
...
| Converge | **29 ms** | eBPF drain + regs + FDs + dirty + non-lazy + VMA diff |
| **Total source frozen** | **55 ms** | 26ms seize + 29ms converge |
...
| `criu/page-xfer.c` | Page server, 8-stream bulk, converge capture |
| `criu/cow-dump.c` | WP_ASYNC tracking, userfaultfd injection |
| `criu/cr-dump.c` | Seize orchestration, early resume, BPF start |
| `criu/cr-restore.c` | Restore + converge state application |
...
3. **Client connections**: closed at seize (`--tcp-close`).
4. **x86_64**: tested on aarch64. x86_64 expected to work but not yet
   validated at scale.
5. **aarch64 + x86_64**: converge register application supports both.
