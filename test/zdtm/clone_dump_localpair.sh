#!/bin/bash
#
# Two-netns harness for --clone-dump ZDTM tests on a single machine.
#
# Sets up two network namespaces + two independent pid-ns shells (primary
# and replica), so --clone-dump's two-endpoint + PID-reuse model can be
...
IMAGES_DIR="/tmp/clone_localpair_$TEST_NAME"
PRIMARY_NS="clone_primary"
REPLICA_NS="clone_replica"
PRIMARY_IP="10.200.0.1"
REPLICA_IP="10.200.0.2"
PRIMARY_VETH="vclone_p"
REPLICA_VETH="vclone_r"
PORT=27123
...
# Some tests (e.g. clone_dump_file_backed) declare a mandatory --test_dir
