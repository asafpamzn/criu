...
const char *test_doc = "--clone-dump with MAP_PRIVATE file-backed region: writes "
		       "to the mapping create private CLONE pages that must be "
		       "captured. A background writer keeps dirtying pages "
		       "throughout dump. Post-restore verification: pages are "
		       "filled with the writer's marker (i.e. CLONE copies were "
...
int main(int argc, char **argv)
{
	...
	/* Trigger initial CLONE copies so the region has private pages before dump. */
	memset(mem, WRITER_MARKER, sz);
