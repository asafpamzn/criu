...
const char *test_doc = "--cow-dump shrinkage test: two equally-sized anon-private "
		       ...
		       "(cow_handle_remove_event) at working-set scale.";
const char *test_author = "Asaf Pamuk <asafp@anthropic.com>";

/*
 * Default 256 MB total (128 MB kept + 128 MB dropped) for CI.
 * Override at build time with -DCOW_MUNMAP_HALF_MB=N for scale runs
...
int main(int argc, char **argv)
{
	...
	test_msg("cow_dump_munmap_half: TOTAL=%u MB (kept=%zu MB, drop=%zu MB)\n",
		 (unsigned)TOTAL_MB,
	...
	drain_ms = DRAIN_TIMEOUT_MS_PER_GB *
		   ((HALF_BYTES >> 30) + 1);
	if (cow_wait_for_drain(kept_region, HALF_BYTES,
	...
	if (err) {
		fail("cow_dump_munmap_half verification failed");
		return 1;
	}
