# CLONE Unit Test Implementation Guide

## Overview

This guide is for implementing standalone unit tests for the CLONE dump components.
...
`test/unit/clone/` and are built/run separately from the main CRIU build.
...
Create `test/unit/clone/Makefile` that:
...
Key challenge: The CLONE code uses CRIU internal helpers (`xmalloc`, `xzalloc`,
...
**File under test:** `criu/include/clone/clone-batch-bitmap.h` (header-only, inline)
...
void test_set_range(void)
{
	...
	clone_batch_bitmap_zero(&bm);
	clone_batch_bitmap_set_range(&bm, 60, 10); /* spans word 0-1 boundary */
...
void test_edge_cases(void)
{
	...
	clone_batch_bitmap_zero(&bm);
	clone_batch_bitmap_set_range(&bm, 250, 100); /* goes past 256 */
...
**File under test:** `criu/include/clone/spsc-queue.h` (macro-based)
...
**File under test:** `criu/include/clone/mpsc-queue.h`
...
**File under test:** `criu/clone/page-pool.c`
...
/* 6. get_chunk returns CLONE_ALLOC_BATCH (256) pages */
```

Build note: compile with `-DCLONE_CHUNK_SIZE=(4*1024*1024)` for tests to use
...
**File under test:** `criu/clone/unmapped-tracker.c`
...
**File under test:** `criu/clone/page-state-tracker.c`
...
# Tests that link against CLONE source files
test_page_pool: test_page_pool.c $(CRIU_SRC)/clone/page-pool.c
	$(CC) $(CFLAGS) -o $@ $< $(CRIU_SRC)/clone/page-pool.c -lpthread

test_unmapped_tracker: test_unmapped_tracker.c $(CRIU_SRC)/clone/unmapped-tracker.c
	$(CC) $(CFLAGS) -o $@ $< $(CRIU_SRC)/clone/unmapped-tracker.c -lpthread
...
1. **Include path issues:** The CLONE headers include other CRIU headers like
