# COW System Test Implementation Guide

## Overview

This guide is for implementing end-to-end system tests that exercise the full
COW dump/restore pipeline. These tests require:
...
## How COW Dump Works (E2E Flow)

The COW dump is a multi-phase live migration:
...
sudo ./criu/criu dump \
  --tree $PID \
  --images-dir /tmp/cow-test-images \
  --cow-dump \
  --lazy-pages \
  --address 127.0.0.1 \
  --port 27000 \
  --leave-running \     # Process keeps running during COW
  --tcp-close \
  -v4 -o /tmp/cow-test-images/dump.log
...
sudo ./criu/criu lazy-pages \
  ...
  --port 27000 \
  --cow-dump \
  -v4 -o /tmp/cow-test-images/lazy-pages.log &
...
sudo ./criu/criu restore \
  ...
  --cow-dump \
  --restore-detached \
  -v4 -o /tmp/cow-test-images/restore.log
```
...
├── lib/
│   └── cow_test_lib.sh    # Shared shell helpers
├── test_basic_cow.sh       # Test wrapper script
├── test_basic_cow.c        # Test process binary
├── test_write_storm.sh
├── test_write_storm.c
├── test_mmap_munmap.sh
├── test_mmap_munmap.c
...
## Shared Test Library (cow_test_lib.sh)

```bash
#!/bin/bash
# Common helpers for COW system tests
...
setup_images_dir() {
    IMAGES_DIR=$(mktemp -d /tmp/cow-test-XXXXXX)
...
start_test_process() {
    ...
    local ready_file="/tmp/cow-test-ready-$TEST_PID"
...
# Run the full COW dump + restore cycle (local, same machine)
cow_dump_restore_local() {
    local pid=$1
    local verify_cmd="${2:-}"
    ...
    # Kill the original process (it was left running for COW)
...
cow_dump_kill_restore() {
    local pid=$1

    # Dump with COW (no --leave-running, process is killed after dump)
    if ! sudo "$CRIU_BIN" dump \
        --tree $pid \
        --images-dir "$IMAGES_DIR" \
        --cow-dump \
...
## Test 1: test_basic_cow.c — Basic Write Tracking

Tests that pages written during COW phase are correctly captured.

```c
/*
 * Test: Basic COW write tracking
 *
...
int main(int argc, char **argv)
{
    ...
    snprintf(ready_file, sizeof(ready_file), "/tmp/cow-test-ready-%d", getpid());
    ...
    /* Write to every 4th page (COW writes) */
    for (i = 0; i < NUM_PAGES; i += 4) {
        memset(mem + i * PAGE_SZ, PATTERN_COW, PAGE_SZ);
    }
    ...
    for (i = 0; i < NUM_PAGES; i++) {
        unsigned char expected = (i % 4 == 0) ? PATTERN_COW : PATTERN_INIT;
...
Wrapper script (`test_basic_cow.sh`):
```bash
#!/bin/bash
source "$(dirname $0)/lib/cow_test_lib.sh"
setup_images_dir

# Build if needed
make -C "$(dirname $0)" test_basic_cow 2>/dev/null
...
# Dump
cow_dump_restore_local $TEST_PID

# After restore, send SIGUSR2 to trigger verification
NEW_PID=$(pgrep -f test_basic_cow | grep -v $$)
if [ -z "$NEW_PID" ]; then
...
wait $NEW_PID
RC=$?
[ $RC -eq 0 ] && pass "basic_cow" || fail "basic_cow (exit=$RC)"
```

## Test 2: test_write_storm.c — High Write Rate During COW

```c
/*
 * Test: Multiple threads writing rapidly during COW phase.
...
## Test 3: test_mmap_munmap.c — Dynamic Memory During COW

```c
/*
 * Test: mmap/munmap during COW phase.
 *
 ...
 * - During COW: threads increment counters
...
```c
/*
 * Test: MAP_PRIVATE file-backed mappings with COW.
...
TESTS := test_basic_cow test_write_storm test_mmap_munmap \
...
```c
while (!access("/tmp/cow-test-phase1", F_OK)) sleep_us(1000);
```
...
For COW tests in ZDTM style, you'd add to `.desc`:
```
{'flags': 'cow-dump lazy'}
```

But since `zdtm.py` may not support `--cow-dump` yet, the standalone script
...
```bash
# Check dump log
sudo cat /tmp/cow-test-*/dump.log | grep -i error

# Check lazy-pages log
sudo cat /tmp/cow-test-*/lazy-pages.log | grep -i error

# Check restore log
sudo cat /tmp/cow-test-*/restore.log | grep -i error

# Enable page state tracker for detailed debugging
# Edit criu/include/cow/cow-conf.h:
...
1. `test_basic_cow` — simplest E2E, validates core WP → snapshot → restore
