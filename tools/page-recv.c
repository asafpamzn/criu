...
static void *stream_worker(void *arg)
{
	...
	while (1) {
		...
		/* Handle converge register data */
		if (cmd == PS_IOV_CONVERGE_REGS) {
			...
			snprintf(path, sizeof(path),
				 "%s/converge_regs.dat", g_images_dir);
			fd = open(path,
			...
			if (fd >= 0) {
				...
				if (w1 < 0 || w2 < 0)
					fprintf(stderr, "stream %d: "
						"converge_regs.dat write "
						"error\n", ctx->id);
			}
			fprintf(stderr, "stream %d: converge regs "
				"received (%d threads)\n",
				ctx->id, nr_threads);
			free(tregs);
			continue;
		...
		if (cmd == PS_IOV_CONVERGE_FDS) {
			...
			__sync_fetch_and_add(&ctx->bytes_received,
					     fds_sz);
			snprintf(path, sizeof(path),
				 "%s/converge_fds.dat", g_images_dir);
			fd = open(path,
				  O_CREAT | O_WRONLY | O_TRUNC, 0644);
			if (fd >= 0) {
				int cnt = nr_fds;
				...
				if (w1 < 0 || w2 < 0)
					fprintf(stderr, "stream %d: "
						"converge_fds.dat write "
						"error\n", ctx->id);
			}
			fprintf(stderr, "stream %d: converge FDs "
				"received (%d fds)\n",
				ctx->id, nr_fds);
			free(tfds);
			continue;
		...
		if (cmd == PS_IOV_CONVERGE_SIGACTS) {
			/* 64 signals × 4 u64 (handler, flags, restorer, mask) */
			size_t sig_sz = CONVERGE_NSIG * 4 *
			...
			__sync_fetch_and_add(&ctx->bytes_received,
					     sig_sz);
			snprintf(path, sizeof(path),
				 "%s/converge_sigacts.dat", g_images_dir);
			fd = open(path,
				  O_CREAT | O_WRONLY | O_TRUNC, 0644);
			if (fd >= 0) {
				ssize_t w;
				...
				if (w < 0)
					fprintf(stderr, "stream %d: "
						"converge_sigacts.dat write "
						"error\n", ctx->id);
			}
			fprintf(stderr, "stream %d: converge sigacts "
		...
		if (cmd == PS_IOV_CONVERGE_ITIMERS) {
			/* 3 itimers × 4 longs = 96 bytes */
			size_t sz = 3 * 4 * sizeof(long);
			char *buf;
			char path[256];
			int fd;

			buf = malloc(sz);
			if (!buf) { ctx->error = 1; break; }
			if (recv_full(ctx->sk, buf, sz) < 0) {
				free(buf);
				ctx->error = 1;
				break;
			}
			__sync_fetch_and_add(&ctx->bytes_received, sz);
			snprintf(path, sizeof(path),
				 "%s/converge_itimers.dat", g_images_dir);
			fd = open(path,
				  O_CREAT | O_WRONLY | O_TRUNC, 0644);
			if (fd >= 0) {
			...
			fprintf(stderr, "stream %d: converge itimers "
				"received (%zu bytes)\n", ctx->id, sz);
			free(buf);
			continue;
		}

		if (cmd == PS_IOV_CONVERGE_MISC) {
			/* 6 unsigned longs = 48 bytes */
			size_t sz = 6 * sizeof(unsigned long);
			char *buf;
			char path[256];
			int fd;
			...
			__sync_fetch_and_add(&ctx->bytes_received, sz);
			snprintf(path, sizeof(path),
				 "%s/converge_misc.dat", g_images_dir);
			fd = open(path,
				  O_CREAT | O_WRONLY | O_TRUNC, 0644);
			if (fd >= 0) {
				if (write(fd, buf, sz) < 0)
			...
			fprintf(stderr, "stream %d: converge misc "
				"received (%zu bytes)\n", ctx->id, sz);
			free(buf);
			continue;
		}

		if (cmd == PS_IOV_CONVERGE_THREAD_STATE) {
			/* npages = nr_threads, each entry ~56 bytes */
			size_t entry_sz = 56; /* sizeof converge_thread_extra */
			size_t sz = npages * entry_sz;
			char *buf;
			char path[256];
			int fd;
			...
			fprintf(stderr, "stream %d: converge thread_state "
				"(%u threads, %zu bytes)\n",
				ctx->id, (unsigned)npages, sz);
			free(buf);
			continue;
		}
		...
		/* Sanity check: reject absurd page counts (>16GB per message) */
		if (npages > (1ULL << 22)) {
			fprintf(stderr, "stream %d: npages too large (%llu), "
				"likely corrupted header\n",
				ctx->id, npages);
			ctx->error = 1;
			break;
		}
		...
		if (data_len > data_buf_sz) {
			char *new_buf = realloc(data_buf, data_len);
			if (!new_buf) {
				ctx->error = 1;
				break;
			}
	...
	/* NULL-guard: set to NULL after free to prevent double-free
	 * from tcache corruption during realloc cycles */
	if (data_buf) { free(data_buf); data_buf = NULL; }
	if (comp_buf) { free(comp_buf); comp_buf = NULL; }
	close(ctx->sk);
	return NULL;
}
